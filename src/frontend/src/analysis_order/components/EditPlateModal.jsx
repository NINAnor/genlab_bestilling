...
/**
 * Modal for editing an analysis plate's name, analysis type, and markers.
 */
export default function EditPlateModal({ isOpen, onClose, plate, onSuccess }) {
  ...
  useEffect(() => {
    if (isOpen && plate) {
      ...
      setAnalysisTypeId(plate.analysis_type ? String(plate.analysis_type) : '');
      setSelectedMarkers(plate.marker_names || []);
    }
  }, [isOpen, plate]);
  ...
  }, [analysisTypeId, plate?.analysis_type]);
  ...
  const isPending = updatePlate.isPending;
...
EditPlateModal.propTypes = {
  ...
  plate: PropTypes.shape({
    ...
    marker_names: PropTypes.arrayOf(PropTypes.string),
  }),
