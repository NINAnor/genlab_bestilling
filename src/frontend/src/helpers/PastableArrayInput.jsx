    ...
    <>
      ...
      <div className="flex gap-2 mt-2">
        ...
        {state.value.length > 0 && (
          <Button className="btn bg-yellow-200" onClick={() => handleChange([])}>
            Clear
          </Button>
        )}
      </div>
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex w-screen items-center justify-center p-4 ">
          <DialogPanel className="max-w-5xl bg-white py-5 px-5">
            ...
            <Description>
              Copy and Paste the column you want to insert, new lines are used as separator
            </Description>
            <Textarea
            ...
            {invalid && (
              <div className="bg-red-400 p-2 rounded mb-3">
                It seems that you have pasted more than a single excel column, please check again
              </div>
            )}
            <div className="flex justify-start gap-4">
              ...
              <Button className="btn block bg-yellow-200" onClick={() => setInput('')}>
                Clear
              </Button>
              <Button className="btn block bg-yellow-200 ml-auto" onClick={() => setIsOpen(false)}>
                Cancel
              </Button>
            </div>
          </DialogPanel>
